        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-8">
                <form action="{{ route('post.store') }}" enctype="multipart/form-data" method="post">
                    @csrf 
                    {{-- @csrf = thêm input hidden chứa token chống CSRF vào form.
                    Giúp Laravel biết request này là hợp lệ, không phải request giả mạo. --}}
                    <!-- Image -->
                    <div>
                    ...
                    <div class="mt-4">
                        ...
                        <x-input-textarea id="content" class="block mt-1" name="content">{{ old('content')}}</x-input-textarea>
                        <x-input-error :messages="$errors->get('title')" class="mt-2"/>
                    </div>
